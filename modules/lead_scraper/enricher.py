...
# Taille des batchs par défaut
DEFAULT_BATCH_SIZE = int(os.getenv('ENRICHMENT_BATCH_SIZE', '10'))
...
class Enricher:
    ...
    Usage:
        enricher = Enricher(pappers_client=PappersClient())
    ...
    def enrich_batch(
        ...
        Args:
            ...
            batch_size: Taille des batchs pour enrichissement (défaut: DEFAULT_BATCH_SIZE)
        ...
        credits_info = ""
        if self.pappers_client:
            credits_info = f"Crédits Pappers utilisés: {self.pappers_client.get_request_count()}"
