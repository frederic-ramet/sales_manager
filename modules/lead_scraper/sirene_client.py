...
Permet de rechercher des entreprises françaises avec filtres avancés.

API gratuite, sans clé requise.
Rate limit: 400 requêtes/minute.
"""
...
SIRENE_RATE_LIMIT = 400  # Requêtes par minute
MAX_RESULTS = 500
DEFAULT_PER_PAGE = 25
MAX_PER_PAGE = 25
REQUEST_TIMEOUT = 30
...
class SireneClient:
    ...
    def __init__(self):
        ...
        self.rate_limit = SIRENE_RATE_LIMIT
        self.request_times: List[float] = []
        self.client = httpx.Client(
            timeout=REQUEST_TIMEOUT,
            headers={
                "User-Agent": "SalesOps-LeadScraper/1.0 (Python/httpx)",
    ...
    def search_by_siren(self, siren: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Erreur recherche SIREN {siren}: {e}")
            return None
    ...
    def _parse_company(self, result: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {
                ...
                "departement": siege.get("code_postal", "")[:2] if siege.get("code_postal") else "",
    ...
    def test_connection(self) -> bool:
        ...
        """
        try:
            response = self.client.get(f"{self.base_url}/search", params={"q": "test", "per_page": 1})
            response.raise_for_status()
            return True
        except Exception as e:
            logger.error(f"Erreur test connexion SIRENE: {e}")
            return False
