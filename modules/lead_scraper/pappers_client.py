...
API payante (100 crédits/mois gratuit).
"""
...
class PappersClient:
    ...
    Permet d'enrichir les données entreprises avec:
    - Dirigeants (nom, prénom, fonction)
    - Contacts (email, téléphone, site web)
    - Finances (chiffre d'affaires)

    Usage:
        with PappersClient() as client:
            company_data = client.get_company("123456789")
            enriched = client.enrich_company(sirene_data)
    """
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        Args:
            api_key: Clé API Pappers (utilise PAPPERS_API_KEY si None)
        """
    ...
    def enrich_company(self, company: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Merge avec les données existantes (ne pas écraser si déjà présent)
    ...
    def test_connection(self) -> bool:
        ...
        """
        try:
            # Utiliser un SIREN connu (SIREN de la SNCF)
            result = self.get_company("552049447")
            return result is not None
        except Exception as e:
            logger.error(f"Erreur test connexion Pappers: {e}")
            return False
