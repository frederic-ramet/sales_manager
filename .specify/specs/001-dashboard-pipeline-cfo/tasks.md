...
- [x] Créer structure de dossiers (`core/`, `tests/`, `.specify/`)
- [x] Créer `requirements.txt` avec dépendances
- [x] Setup `.env.example` et `.gitignore`
...
- [x] Créer `core/asana_client.py`
- [x] Implémenter authentification PAT
- [x] Implémenter `get_project_tasks()` avec opt_fields complets
- [x] Implémenter `get_custom_field_gids()` pour validation
- [ ] Tester avec le vrai projet Asana
- [x] Gérer rate limits et retry
...
- [x] Créer `core/pipeline.py`
- [x] Implémenter `parse_tasks_to_dataframe()`
- [x] Implémenter `calculate_metrics()` :
  - Probabilité basée sur confidence score
  - Revenue pondéré
  - Marge %
  - Dimensions temporelles (année, trimestre, mois)
  - Flags scénarios (conservateur, probable)
- [x] Helper `_parse_custom_fields()` robuste
- [x] Tests unitaires avec données mockées
...
- [x] Créer `core/sheets_sync.py`
- [x] Implémenter `sync_pipeline()` avec création des 4 onglets :
  - Pipeline complet
  - Scénario conservateur
  - Scénario probable
  - Config/logs
- [x] Implémenter `_write_to_sheet()` (clear + write)
- [x] Gestion des erreurs (sheet not found, permissions)
...
- [x] Documenter mapping custom fields attendus
...
**Critères de succès** :
- [x] Sync manuelle fonctionne via interface Streamlit
- [x] Les 4 onglets sont créés dans Google Sheets
- [x] Calculs financiers corrects (vérifiés manuellement)
- [x] Gestion d'erreurs (mauvais token, sheet inexistant, etc.)
- [x] Documentation claire pour un nouvel utilisateur
